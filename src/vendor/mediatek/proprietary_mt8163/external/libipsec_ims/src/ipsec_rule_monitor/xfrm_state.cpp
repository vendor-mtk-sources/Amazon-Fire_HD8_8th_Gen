...
int xfrm_state_process(struct nlmsghdr *n)
{
	...
	if (n->nlmsg_type == XFRM_MSG_DELSA) {
		xsid = static_cast<struct xfrm_usersa_id *>NLMSG_DATA(n);
		len -= NLMSG_SPACE(sizeof(*xsid));
	} else if (n->nlmsg_type == XFRM_MSG_EXPIRE) {
		xexp = static_cast<struct xfrm_user_expire *>NLMSG_DATA(n);
		xsinfo = &xexp->state;
		len -= NLMSG_SPACE(sizeof(*xexp));
	} else if (n->nlmsg_type == XFRM_MSG_FLUSHSA) {
		xsf = static_cast<struct xfrm_usersa_flush *>NLMSG_DATA(n);
		len -= NLMSG_SPACE(sizeof(*xsf));
	} else {
		xexp = NULL;
		xsinfo = static_cast<struct xfrm_usersa_info *>NLMSG_DATA(n);
