		...
		for (i = 0, j = 0 ; i < prNetworkAddressList->u4AddressCount ; i++) {
			...
			if (prNetworkAddress->u2AddressType == PARAM_PROTOCOL_ID_TCP_IP &&
				...
				DBGLOG(OID, INFO, "prNetAddrIpv6->addr:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x\n",
						pucBuf[0], pucBuf[1], pucBuf[2], pucBuf[3], pucBuf[8], pucBuf[9],
						pucBuf[10], pucBuf[11], pucBuf[12], pucBuf[13], pucBuf[14], pucBuf[15]);
				prNetworkAddress = (P_PARAM_NETWORK_ADDRESS) ((ULONG)prNetworkAddress +
						prNetworkAddress->u2AddressLength + OFFSET_OF(PARAM_NETWORK_ADDRESS, aucAddress));
				j++;
			}
		     ...
		     IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	prBssInfo = &(prAdapter->rWifiVar.arBssInfo[prParamMacOui->ucBssIndex]);
	if (!prBssInfo) {
		 ...
		 OUT uint32_t *pu4QueryInfoLen)
{
	...
	do {
		...
		DBGLOG(REQ, TRACE, "dump rssi statistics info  [%d,%d],[%d,%d],[%d,%d],[%d,%d]\n",
			...
			prQueryRssiStatistics->arRxRssiStatistics.ucM1Rcpi,
			  ...
			  IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	do {
		...
		prBssInfo = &prAdapter->rWifiVar.arBssInfo[NETWORK_TYPE_AIS_INDEX];
		if (!prBssInfo || prBssInfo->eConnectionState != PARAM_MEDIA_STATE_CONNECTED)
			break;
			  ...
			  IN PVOID pvSetBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	do {
		...
		prAdapter->fgIsTest1xTx = *(PINT_8) pvSetBuffer;
		DBGLOG(OID, WARN, "rxIsTest %u\n", prAdapter->fgIsTest1xTx);
	} while (FALSE);
	return rResult;
}
